...
import application.models.ModelBlok;
import application.models.ModelKamar;
...
//            curentStage.setMaximized(true);
    ...
    void ambahKamar(ActionEvent event) {
        ...
        if (tambahkamarTxt.getText().trim().isEmpty()){
            template_error.warning("Peringatan", "tolong lengkapi form tambah kamar");
        }else{
            try{
    ...
    void tambahBlokBaru(ActionEvent event) {
        int rs = 0;
        if (tambakBlokTxt.getText().trim().isEmpty()){
            template_error.warning("Peringatan", "silahkan isi nam blok");
        }else{
            try{
